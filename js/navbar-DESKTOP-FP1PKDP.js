...
const restriveTrackInfo = async (trackId) => {
    ...
    // Se la tracck che sto per riprodurre Ã¨ diversa da quella che sto giÃ  riproducendo
...
// Se le informazioni della traccia ono giÃ  presenti nell'aray globale, non le recupera di nuovo
const playFunction = async (trackId) => {
    _D(1, `trackId: ${trackId}`)

    // Se la traccia che sto per riprodurre Ã¨ diversa da quella che sto giÃ  riproducendo
...
let currentTrackData // Inizializza la variabile che conterrÃ  i dati della traccia corrente recuperati tramite il fetching dell'api
...
setTimeout(() => {
    ...
    Promise.all([retrieveTrackInfo(99710032)]).
        then(() => {
            playFunction()
            updatePlayerBar()
        })
